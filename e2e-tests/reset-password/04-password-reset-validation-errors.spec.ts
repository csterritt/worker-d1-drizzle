...
test('shows invalid token page when no token provided', async ({ page }) => {
  ...
  expect(
    await page.locator('[data-testid="request-new-reset-link"]').isVisible()
  ).toBe(true)
  expect(
    await page
      .locator('[data-testid="back-to-sign-in-from-invalid"]')
      .isVisible()
  ).toBe(true)
})
...
test('can navigate from invalid token page', async ({ page }) => {
  ...
  await clickLink(page, 'request-new-reset-link')
  expect(page.url()).toContain('/auth/forgot-password')
