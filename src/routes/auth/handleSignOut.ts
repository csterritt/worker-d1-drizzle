...
export const handleSignOut = (app: Hono<{ Bindings: Bindings }>): void => {
  app.post('/auth/sign-out', async (c) => {
    try {
      ...
      fallbackResponse.headers.append('Set-Cookie', 'better-auth.session_token=; Path=/; HttpOnly; SameSite=lax; Max-Age=0')
      fallbackResponse.headers.append('Set-Cookie', 'better-auth.session_data=; Path=/; HttpOnly; SameSite=lax; Max-Age=0')
